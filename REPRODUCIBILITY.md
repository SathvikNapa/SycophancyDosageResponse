...
| `app:fragmentation` (appendix) | `analysis/fragmentation_vs_uncertainty.py` — all three benchmarks, all six models; correlates baseline entropy against whether a T0-correct question ever lost consensus | `experiment_out/*/base_experiment_metadata.pkl` + `experiment_out/*/{,<dataset>/}reasoning_calibrated_bin/*_cross_turn.pkl` (joined on `query[:80]`, since the cross_turn pkl truncates the query field) |
