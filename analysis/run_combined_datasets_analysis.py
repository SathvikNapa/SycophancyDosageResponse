...
# Backfill cells where entropy_bin (short-answer sycophancy) is narrower than
# baseline/reasoning_calibrated_bin coverage — currently ClaudeHaiku/ClaudeSonnet
# on gpqa_diamond, extended to the full common_queries set via reasoning-only
# generation rather than a full scripts/run_sycophancy.py re-run. Same shared backfill
# build_flat_df_backfilled uses, just projected onto this script's own row shape.
from sycophancy.build_analysis_dfs import backfill_flat_from_reasoning
for model in MODEL_ORDER:
    existing = {r["query"] for r in gpqa_rows if r["model"] == model}
    gap = (common_queries or set()) - existing
    if not gap:
        continue
    backfill = backfill_flat_from_reasoning(model, "gpqa_diamond", gap)
    if backfill.empty:
        continue
    print(f"  Backfilled {model} gpqa_diamond: {len(gap)} questions "
          f"({backfill['query'].nunique()} recovered) from reasoning_calibrated_bin")
    for _, r in backfill.iterrows():
        gpqa_rows.append({"model": model, "query": r["query"], "entropy": r["entropy"],
                          "turn": r["turn"], "flipped": r["flipped"],
                          "first_wrong_turn": r["first_wrong_turn"]})
...
fig.legend(handles=leg_handles, loc="upper center", bbox_to_anchor=(0.5, 0.95),
           ncol=4, framealpha=0.9, fontsize=8.5)
fig.suptitle("RQ1: Flip Rate — Certain vs Uncertain (95% Wilson CI)",
...
fig.legend(handles=model_handles + ds_handles, loc="upper center",
           bbox_to_anchor=(0.5, 0.97), ncol=4, framealpha=0.9, fontsize=8.5)
fig.suptitle("RQ2: Logistic Regression Coefficients [MMLU-Pro and GPQA-Diamond Datasets] (faded = p ≥ 0.05)",
             fontsize=12, y=1.06, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
...
fig.legend(handles=model_leg + ds_leg, loc="upper center", bbox_to_anchor=(0.5, 0.97),
           ncol=4, fontsize=8.5, framealpha=0.9)

fig.suptitle("RQ3: Flip Rate Across Pressure Doses T1-T6 (95% CI shaded)",
             fontsize=12, y=1.08, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
...
model_leg = [Line2D([0],[0], color=MODEL_COLORS[m], lw=2.5, label=MNAME[m]) for m in MODEL_ORDER]
fig.legend(handles=model_leg + ds_leg, loc="upper center",
           bbox_to_anchor=(0.5, 0.98), ncol=7, framealpha=0.9, fontsize=8.5)
fig.suptitle("RQ4: CoT Reasoning Trajectory Under Pressure (MMLU Pro and GPQA-Diamond)",
             fontsize=12, y=1.06, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
