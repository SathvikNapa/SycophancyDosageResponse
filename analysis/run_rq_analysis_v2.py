...
def load_gpqa_reasoning_df(
    ...
    # cross_turn pkls store query truncated to 80 chars (see
    # reasoning_uncertainty.summarise_cross_turn_comparison), while
    # common_queries holds full-length strings from base_experiment_metadata —
    # compare by the same truncated prefix or every row gets filtered out.
    common_prefixes = {cq[:80] for cq in common_queries} if common_queries is not None else None
    rows = []
    for model in models:
        ...
        for pkl in sorted(glob.glob(os.path.join(d, "bin_*_cross_turn.pkl"))):
            ...
            for ct in cross:
                q = ct.get("query","")
                if common_prefixes is not None and q not in common_prefixes:
                    continue
                rows.append({
                    "model":                         model,
...
df = build_flat_df_backfilled(restrict_queries={"gpqa_diamond": common_gpqa})
pressure = df[df["turn"] >= 1].copy()

print("Loading MMLU reasoning df...")
from sycophancy.build_analysis_dfs import build_reasoning_df, REASONING_TRACE
...
fig.legend(handles, labels_leg, loc="upper center", bbox_to_anchor=(0.5, 0.95),
           ncol=3, framealpha=0.9, edgecolor="#ccc", title="Hardness bin")
fig.suptitle("Question Distribution by Calibrated Hardness Bin (Easy: p<1/3, Medium: 1/3-2/3, Hard: p>=2/3)",
             fontsize=10, y=1.05, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
...
fig.suptitle("RQ1: Flip Rate by Calibrated Hardness (95% Wilson CI)",
             fontsize=12, y=1.05, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
...
fig, axes = plt.subplots(1, 2, figsize=(13, 4.5), sharey=True, sharex=True)
...
for ax, ds in zip(axes, DATASET_ORDER):
    ...
    ax.set_yticklabels(MODEL_LABELS, fontsize=13.5)
...
#                bbox_to_anchor=(1.0, 1.12), ncol=3,
#                framealpha=0.9, edgecolor="#ccc")
fig.legend(handles=handles, loc="upper center",
           bbox_to_anchor=(0.5, 0.98), ncol=3,
           framealpha=0.9, edgecolor="#ccc")
fig.suptitle("RQ2: Logistic Regression Coefficients: calibrated hardness (faded = p >= 0.05)",
             fontsize=13, y=1.02, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
out = os.path.join(PLOT_DIR, "rq2_coefficients_v2.pdf")
fig.savefig(out, bbox_inches="tight")
plt.close(fig)
print(f"  Saved {out}")

# ═══════════════════════════════════════════════════════════════════════════
# RQ2b — Fig 3/4 reconstruction: predicted probability + empirical interaction
# on raw baseline entropy (not calibrated hardness), per model x dataset.
# No generating script for the originals survived — this refits directly.
# ═══════════════════════════════════════════════════════════════════════════

print("\n=== RQ2b (Fig 3/4 reconstruction) ===")
...
mmlu_pressure = pressure[pressure["dataset"]=="mmlu_pro"].dropna(subset=["entropy","flipped"])
turns_grid = np.arange(1, 7)

# --- Fig 3: predicted flip probability at H=0 (certain) vs H=-1.5 (uncertain) ---
fig, axes = plt.subplots(1, 5, figsize=(18, 4), sharey=True)
for ax, model, mlabel in zip(axes, MODEL_ORDER, MODEL_LABELS):
    m = mmlu_pressure[mmlu_pressure["model"]==model]
    ...
    try:
        ...
        for H, ls, hlabel in [(0.0, "-", "certain ($H=0$)"), (-1.5, "--", "uncertain ($H=-1.5$)")]:
            grid = pd.DataFrame({"turn": turns_grid, "entropy": H})
            pred = mod.predict(grid)
            ax.plot(turns_grid, pred, color=color, linestyle=ls, marker="o",
                    markersize=4, linewidth=1.8, label=hlabel)
    except Exception as e:
        print(f"  {model}: ERROR {e}")
    ax.set_xticks(turns_grid)
    ax.set_xticklabels([f"T{t}" for t in turns_grid])
    ax.set_title(mlabel.replace("\n", " "), fontsize=10)
    ax.set_ylim(0, 1)
    ax.yaxis.set_major_formatter(mticker.PercentFormatter(1.0))
    ax.xaxis.grid(False); ax.yaxis.grid(True, alpha=0.3, linestyle=":")

axes[0].set_ylabel("Predicted flip probability")
handles, labels_leg = axes[0].get_legend_handles_labels()
fig.legend(handles, labels_leg, loc="upper center", bbox_to_anchor=(0.5, 0.99),
           ncol=2, framealpha=0.9, edgecolor="#ccc")
fig.suptitle("Predicted Flip Probability by Baseline Uncertainty (MMLU-Pro)",
             fontsize=13, y=1.12, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
out = os.path.join(PLOT_DIR, "rq2_interaction.pdf")
fig.savefig(out, bbox_inches="tight")
plt.close(fig)
print(f"  Saved {out}")

# --- Fig 4: empirical flip rate by entropy quantile bin, at T1/T3/T6 ---
TURN_COLORS = {1: "#4B0082", 3: "#2A9D8F", 6: "#8AB17D"}
fig, axes = plt.subplots(1, 5, figsize=(18, 4), sharey=True)
for ax, model, mlabel in zip(axes, MODEL_ORDER, MODEL_LABELS):
    m = mmlu_pressure[mmlu_pressure["model"]==model].dropna(subset=["entropy"])
    color = MODEL_COLORS[model]
    q_level = m.drop_duplicates(subset=["query"])[["query","entropy"]]
    if len(q_level) < 10:
        ax.set_title(f"{mlabel}\n(insufficient data)", fontsize=9)
        continue
    try:
        # H=0 (exactly certain) is its own bin; H<0 (uncertain) mass gets
        # quantile-binned separately, since a model with lots of certain
        # questions has almost no spread left to quantile over otherwise.
        uncertain = q_level[q_level["entropy"] < 0].copy()
        certain_q = q_level[q_level["entropy"] == 0]["query"].tolist()
        bin_defs = []  # (query_list, xtick_label, sort_key)
        n_distinct = uncertain["entropy"].nunique()
        if n_distinct >= 2:
            n_q = min(4, n_distinct)
            uncertain["bin"] = pd.qcut(uncertain["entropy"], q=n_q, duplicates="drop")
            for b in sorted(uncertain["bin"].dropna().unique(), key=lambda iv: iv.left):
                qs = uncertain.loc[uncertain["bin"]==b, "query"].tolist()
                bin_defs.append((qs, f"{b.left:.2f}", b.left))
        elif n_distinct == 1:
            bin_defs.append((uncertain["query"].tolist(),
                              f"{uncertain['entropy'].iloc[0]:.2f}", uncertain["entropy"].iloc[0]))
        if certain_q:
            bin_defs.append((certain_q, "H=0", 0.0))

        xs = np.arange(len(bin_defs))
        for t in (1, 3, 6):
            grp = m[m["turn"]==t]
            ys = [grp[grp["query"].isin(qs)]["flipped"].mean()*100 if qs else np.nan
                  for qs, _, _ in bin_defs]
            ax.plot(xs, ys, color=TURN_COLORS[t], marker="o", markersize=4,
                    linewidth=1.8, label=f"$T_{t}$")
        ax.set_xticks(xs)
        ax.set_xticklabels([lbl for _, lbl, _ in bin_defs], rotation=30, ha="right", fontsize=7)
    except Exception as e:
        print(f"  {model}: ERROR {e}")
    ax.set_title(mlabel.replace("\n", " "), fontsize=10)
    ax.set_xlabel("Baseline entropy $H$", fontsize=9)
    ax.xaxis.grid(False); ax.yaxis.grid(True, alpha=0.3, linestyle=":")
...
fig.legend(handles, labels_leg, loc="upper center", bbox_to_anchor=(0.5, 0.99),
           ncol=3, framealpha=0.9, edgecolor="#ccc")
fig.suptitle("Empirical Flip Rate by Baseline-Entropy Bin (MMLU-Pro)",
             fontsize=13, y=1.12, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
out = os.path.join(PLOT_DIR, "rq2_empirical_interaction.pdf")
fig.savefig(out, bbox_inches="tight")
plt.close(fig)
print(f"  Saved {out}")
...
# Shared legend — model names + Resister/Capitulator key merged into one legend
from matplotlib.lines import Line2D
handles, labels = axes[0].get_legend_handles_labels()
h2, l2 = axes[1].get_legend_handles_labels()
all_h = {l: h for l, h in zip(labels+l2, handles+h2)}
leg_extra = [Line2D([0],[0],color="#555555",lw=2,label="Resister (solid)"),
             Line2D([0],[0],color="#555555",lw=2,linestyle="--",label="Capitulator (dashed)")]
fig.legend(list(all_h.values())+leg_extra, list(all_h.keys())+["Resister (solid)","Capitulator (dashed)"],
           loc="upper center", bbox_to_anchor=(0.5, 0.97), ncol=4,
           framealpha=0.9, edgecolor="#ccc", fontsize=8.5)

fig.suptitle("RQ3: Flip Rate Across Pressure Doses T1-T6 (95% CI shaded)",
             fontsize=12, y=1.05, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
out = os.path.join(PLOT_DIR, "rq3_pressure_curves_v2.pdf")
fig.savefig(out, bbox_inches="tight")
plt.close(fig)
print(f"  Saved {out}")
...
fig.suptitle("RQ4: CoT Reasoning Trajectory Under Pressure (Calibrated)",
             fontsize=13, y=1.04, fontfamily="Tahoma", fontweight="bold")
fig.tight_layout()
out = os.path.join(PLOT_DIR, "rq4_trajectory_v2.pdf")
fig.savefig(out, bbox_inches="tight")
plt.close(fig)
print(f"  Saved {out}")
