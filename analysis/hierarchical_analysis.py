...
def plot_results(per_model_results, het_results, model_names, output_dir):
    """Two-panel figure: per-model β_PH forest plot + pooled-estimate comparison.
    Diagnostics (Q, I², tau) are printed to console instead of a third panel —
    dropped from the figure itself since it was illegible with three panels."""
    ...
    re_handle, re_label = ax.get_legend_handles_labels()
    ax.invert_yaxis()

    # ── Panel 2: Comparison of pooled estimates ──
    ax = axes[1]
    labels = ["Intercept-only", "Random-effects"]
    ...
    # Diagnostics that used to be a third text panel — now printed only,
    # since they read better as prose/appendix text than a monospace box.
    print("=" * 65)
    print("Heterogeneity diagnostics (dropped from figure, kept here):")
    print(f"  Cochran's Q = {het_results['Q']:.2f}  (p = {het_results['p_Q']:.4f})")
    print(f"  I² = {het_results['I2']:.1f}%  "
          f"({'Low' if het_results['I2'] < 25 else 'Moderate' if het_results['I2'] < 75 else 'High'} heterogeneity)")
    print(f"  τ (between-model SD) = {het_results['tau']:.4f}")
    print(f"  |pooled β̂_PH| = {abs(het_results['pooled_re']):.4f}")
    print(f"  Ratio τ / |β̂_PH| = {het_results['tau'] / max(abs(het_results['pooled_re']), 1e-8):.2f}")
    print(f"  RE pooled p-value = {het_results['p_re']:.4f} "
          f"({'significant' if het_results['p_re'] < 0.05 else 'NOT significant'} after accounting for heterogeneity)")
    print("=" * 65)

    fig.legend(re_handle, re_label, loc="upper center", bbox_to_anchor=(0.5, 0.98),
               frameon=False, ncol=1, fontsize=9)
    fig.suptitle("Hierarchical Interaction Analysis: Per-Model Heterogeneity in Pressure×Hardness",
                 fontsize=13, y=1.08, fontfamily="Tahoma", fontweight="bold")

    plt.tight_layout()
    out_path = output_dir / "interaction_heterogeneity.pdf"
...
def main():
    ...
    print(f"  Figure: {output_dir / 'interaction_heterogeneity.pdf'}")
