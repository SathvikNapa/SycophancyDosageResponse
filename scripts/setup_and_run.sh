...
# ── 2. Sync Python env (pyproject.toml + uv.lock is the single source of truth) ──
echo "▶ Syncing dependencies (uv sync)..."
(cd "$ROOT_DIR" && uv sync -q)
...
uv run python "$SCRIPT_DIR/run_baseline.py" \
  --model          "$MODEL"          \
...
uv run python "$SCRIPT_DIR/run_sycophancy.py" \
  --model                "$MODEL"                \
...
if [ "$SKIP_REASONING" = false ]; then
  ...
  uv run python "$SCRIPT_DIR/run_reasoning_sycophancy.py" \
    --model                "$MODEL"                \
