...
from sycophancy.calibrator import calibrate_trace
...
from sycophancy.entropy import (
...
from sycophancy.generator import ResponseGenerator
from sycophancy.reasoning_uncertainty import (
    ReasoningTrace,
...
try:
    from sycophancy.sycophancy_dosage import T1, T2, T3, T4, T5, T6
...
def _turn_ckpt_path(checkpoint_dir: str, item_idx: int, sample_idx: int) -> str:
    return os.path.join(checkpoint_dir, f"item_{item_idx:05d}_sample_{sample_idx:02d}_turns.pkl")


def _save_turn_checkpoint(
    checkpoint_dir: str, item_idx: int, sample_idx: int,
    traces: List[ReasoningTrace], messages: List[dict],
) -> None:
    os.makedirs(checkpoint_dir, exist_ok=True)
    path     = _turn_ckpt_path(checkpoint_dir, item_idx, sample_idx)
    tmp_path = path + ".tmp"
    with open(tmp_path, "wb") as f:
        pickle.dump({"traces": traces, "messages": messages}, f)
    os.replace(tmp_path, path)  # atomic — a kill mid-write can't leave a truncated file


def _load_turn_checkpoint(
    checkpoint_dir: str, item_idx: int, sample_idx: int,
) -> Tuple[List[ReasoningTrace], List[dict]]:
    path = _turn_ckpt_path(checkpoint_dir, item_idx, sample_idx)
    ...
    try:
        with open(path, "rb") as f:
            data = pickle.load(f)
        return data.get("traces", []), data.get("messages", [])
    except Exception:
        return [], []  # corrupt checkpoint — rerun this sample from T0


def _clear_turn_checkpoint(checkpoint_dir: str, item_idx: int, sample_idx: int) -> None:
    try:
        os.remove(_turn_ckpt_path(checkpoint_dir, item_idx, sample_idx))
...
async def run_calibrated_conversation(
    ...
    If checkpoint_dir/item_idx/sample_idx are given, the conversation state
    (traces + messages) is persisted to disk after every single turn — not
    just after the whole sample finishes. A kill mid-conversation resumes
    from the next un-run turn instead of redoing turns that already completed
    ...
    can_checkpoint = (
        checkpoint_dir is not None and item_idx is not None and sample_idx is not None
    )
    ...
    if can_checkpoint:
        cached_traces, cached_messages = _load_turn_checkpoint(
            checkpoint_dir, item_idx, sample_idx
        )
        if cached_traces:
            traces, messages = cached_traces, cached_messages

    # T0 — baseline (turn 0). Skip if already checkpointed.
    if len(traces) == 0:
        ...
        if can_checkpoint:
            _save_turn_checkpoint(checkpoint_dir, item_idx, sample_idx, traces, messages)

    # T1..K — pressure doses. turn_number is 1-indexed (T0 is turn 0); skip
    # any turn already present from a loaded checkpoint.
    for dose_idx, ds in enumerate(dose_statements):
        turn_number = dose_idx + 1
        if turn_number < len(traces):
            continue
    ...
    if can_checkpoint:
        _clear_turn_checkpoint(checkpoint_dir, item_idx, sample_idx)
...
def _sample_ckpt_path(checkpoint_dir: str, item_idx: int, sample_idx: int) -> str:
    return os.path.join(checkpoint_dir, f"item_{item_idx:05d}_sample_{sample_idx:02d}.pkl")


async def _run_sample_cached(
    ...
    """
    Runs one (question, sample) conversation across all turns, and persists it
    to disk the moment it finishes — independently of the other n_samples-1
    runs for this item.  This is the granularity a --extend / resume can
    recover: a kill mid-item no longer discards samples that already finished,
    only whichever one was still in flight.
    """
    ckpt_path = (
        _sample_ckpt_path(checkpoint_dir, item_idx, sample_idx)
        if checkpoint_dir is not None and item_idx is not None
        else None
    )
    if ckpt_path and os.path.exists(ckpt_path):
        try:
            ...
            pass  # corrupt checkpoint — rerun this sample

    result = await run_calibrated_conversation(
...
async def run_calibrated_repeated_single(
    ...
    If checkpoint_dir/item_idx are given, each individual sample is cached to
    disk as soon as it finishes (see _run_sample_cached) — a process killed
    mid-item resumes from whichever samples already completed instead of
    re-running all n_samples from scratch.
    """
    tasks = [
        _run_sample_cached(
            checkpoint_dir=checkpoint_dir, item_idx=item_idx, sample_idx=sample_idx,
            sem=sem, rg=rg, item=item, model=model,
            calibrator_model=calibrator_model,
            dose_statements=dose_statements,
            wrong_answer_text=wrong_answer_text,
...
async def run_calibrated_over_items(
    ...
    if wrong_answers is None:
        from sycophancy.sycophancy import pick_wrong_option_from_item
...
def _load_existing_reasoning_bins(out_dir: str) -> Dict[int, List[UncertaintyTrajectory]]:
    """Loads every bin_N_reasoning.pkl already in out_dir, keyed by bin index."""
    existing: Dict[int, List[UncertaintyTrajectory]] = {}
    if not os.path.isdir(out_dir):
        return existing
    for fname in os.listdir(out_dir):
        if fname.startswith("bin_") and fname.endswith("_reasoning.pkl"):
            try:
                bin_idx = int(fname[len("bin_"):-len("_reasoning.pkl")])
            except ValueError:
                continue
            with open(os.path.join(out_dir, fname), "rb") as f:
                existing[bin_idx] = pickle.load(f)
    return existing
...
async def run_calibrated_bins(
    ...
    existing_by_bin: Dict[int, List[UncertaintyTrajectory]] = {}
    seen_queries: set = set()
    if extend:
        existing_by_bin = _load_existing_reasoning_bins(out_dir)
        for trajs in existing_by_bin.values():
            seen_queries.update(t.query for t in trajs)
        print(
            f"{label_prefix}--extend: {len(seen_queries)} questions already have "
            f"reasoning_calibrated data in {out_dir}, will skip those."
        )
    ...
    for bin_idx, items in entropy_bins.items():
        ...
        if extend:
            items = [it for it in items if it.get("query") not in seen_queries]
            if not items:
                print(f"{label_prefix}Bin {bin_idx}: nothing new, skipping.")
                continue
        ...
        new_trajectories = await run_calibrated_over_items(
            items=items,
            rg=rg,
        ...
        # In --extend mode, merge with whatever was already in this bin file
        # rather than overwriting it — re-binning after adding new questions
        # can shuffle which bin index an old question lands in, so the safe
        # merge key is "already-generated trajectories for this bin_idx file",
        # not "items the binner assigned to bin_idx this run".
        trajectories = existing_by_bin.get(bin_idx, []) + new_trajectories
        ...
        print(f"  Saved trajectories -> {out_path}  ({len(trajectories)} total, {len(new_trajectories)} new)")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Skip questions that already have reasoning_calibrated data "
             "(matched by query across all existing bin_N_reasoning.pkl files "
             "in the output dir) and append newly generated results into the "
             "correct bin file instead of overwriting it.",
    )
    p.add_argument(
        "--recalibrate",
        action="store_true",
...
async def main() -> None:
    ...
    if not args.no_clustering:
        try:
            from sycophancy.reasoning_uncertainty import _load_encoder
